...
/* zip64j: re-enabled together with the setjmp below (upstream disabled both
 * for the USE_ZIPMAIN build). */
#if !defined(MACOS) && (!defined(USE_ZIPMAIN) || defined(WINDLL))
...
/* zip64j: upstream arms this jump target only when USE_ZIPMAIN is absent, but
 * ziperr() ends in longjmp(zipdll_error_return, c) whenever WINDLL is defined.
 * The DLL build defines both, so the buffer stayed zeroed and every ZIPERR
 * path jumped to a null target and took the host process down instead of
 * returning a ZE_ code. Arm it whenever ziperr() can jump here. */
#if !defined(MACOS) && (!defined(USE_ZIPMAIN) || defined(WINDLL))
