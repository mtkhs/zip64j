...
/* zip.h defines CR (13). Windows SDK's winnt.h uses CR as an ARM64 bitfield
   member name — token expansion breaks that struct if CR is macro-defined.
   Pull in <windows.h> (the chief source of winnt.h) with CR undefined, then
   restore it afterwards. Placed early so CRT headers that transitively drag
   in winnt.h (e.g. sys/utime.h) also see the undefined state. */
#undef CR
